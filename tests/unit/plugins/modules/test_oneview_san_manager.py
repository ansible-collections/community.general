...
class SanManagerModuleSpec(unittest.TestCase, OneViewBaseTestCase):
    PARAMS_FOR_PRESENT = dict(config="config.json", state="present", data=DEFAULT_SAN_MANAGER_TEMPLATE)

    PARAMS_FOR_CONNECTION_INFORMATION_SET = dict(
        config="config.json", state="connection_information_set", data=DEFAULT_SAN_MANAGER_TEMPLATE.copy()
    )

    PARAMS_WITH_CHANGES = dict(
        config="config.json",
        state="present",
        data=dict(name=DEFAULT_SAN_MANAGER_TEMPLATE["name"], refreshState="RefreshPending"),
    )

    PARAMS_FOR_ABSENT = dict(config="config.json", state="absent", data=dict(name=DEFAULT_SAN_MANAGER_TEMPLATE["name"]))
    ...
    def test_should_add_new_san_manager(self):
        ...
        self.mock_ansible_module.exit_json.assert_called_once_with(
            changed=True, msg=SanManagerModule.MSG_CREATED, ansible_facts=dict(san_manager=DEFAULT_SAN_MANAGER_TEMPLATE)
        )
    ...
    def test_should_not_update_when_data_is_equals(self):
        ...
        self.mock_ansible_module.exit_json.assert_called_once_with(
            changed=False, msg=SanManagerModule.MSG_ALREADY_PRESENT, ansible_facts=dict(san_manager=output_data)
        )
    ...
    def test_update_when_data_has_modified_attributes(self):
        ...
        self.mock_ansible_module.exit_json.assert_called_once_with(
            changed=True, msg=SanManagerModule.MSG_UPDATED, ansible_facts=dict(san_manager=data_merged)
        )
    ...
    def test_should_remove_san_manager(self):
        ...
        self.mock_ansible_module.exit_json.assert_called_once_with(changed=True, msg=SanManagerModule.MSG_DELETED)
    ...
    def test_should_fail_when_name_not_found(self):
        ...
        self.mock_ansible_module.fail_json.assert_called_once_with(
            exception=mock.ANY, msg="The provider 'Brocade Network Advisor' was not found."
        )
