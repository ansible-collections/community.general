...
class PagerDutyTest(unittest.TestCase):
    ...
    def _assert_create_a_maintenance_window_from_header(self, module, url, headers, data=None, method=None):
        self.assertEqual("requester_id", headers.get("From"), "From:requester_id HTTP header not found")
        return object(), {"status": 201}
    ...
    def _assert_create_window_single_service(self, module, url, headers, data=None, method=None):
        ...
        self.assertEqual([{"id": "service_id", "type": "service_reference"}], services)
        return object(), {"status": 201}
    ...
    def test_create_maintenance_window_http_method(self):
        self.pd.create(
            "requester_id", "service", 1, 0, "desc", http_call=self._assert_create_a_maintenance_window_http_method
        )

    def test_create_maintenance_from_header(self):
        self.pd.create(
            "requester_id", "service", 1, 0, "desc", http_call=self._assert_create_a_maintenance_window_from_header
        )

    def test_create_maintenance_compatibility_header(self):
        self.pd.create(
            "requester_id", "service", 1, 0, "desc", http_call=self._assert_create_window_with_v1_compatible_header
        )
    ...
    def test_create_maintenance_for_multiple_services(self):
        self.pd.create(
            ...
            0,
            "desc",
            http_call=self._assert_create_window_multiple_service,
        )
