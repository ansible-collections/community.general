...
DOCUMENTATION = """
    ...
    options:
      ...
      generate_if_missing:
        description:
          - The O(field) specified will be set to a value generated by 'bw generate' if missing.
            If the record itself does not exist, it will be created.
      ...
      set:
        description:
          - The O(field) specified will be set to this value.
            If the record itself does not exist, it will be created.
          - This option is mutually exclusive with O(generate_if_missing).
          - Only V(username), V(password) and V(uris) can be set as defined fields.
            For any other field-name, a custom-field of that name will be created."
        type: str
        version_added: 8.5.0
...
- name: "Get 'password' from all Bitwarden records named 'a_test', create missing records with an autogenerated, 32 character password"
  ansible.builtin.debug:
    msg: >-
      {{ lookup('community.general.bitwarden', 'a_test', field='password', generate_if_missing='--length 32') }}

- name: "Get 'username' from all Bitwarden records named 'a_test', creating missing records with predefined username 'admin'"
  ansible.builtin.debug:
    msg: >-
      {{ lookup('community.general.bitwarden', 'a_test', field='username', set='admin') }}
"""
...
class Bitwarden(object):
    ...
    def _generate_password(self, password_options):
        out, err = self._run(['generate', *password_options.split(' ')])
        if err:
            raise BitwardenException(err)
        return out

    def _edit_record(self, item_id, changed_record):
        b64_record = b64encode(to_bytes(AnsibleJSONEncoder().encode(changed_record)))

        out, err = self._run(['edit', 'item', item_id], stdin=b64_record)
        if err:
            raise BitwardenException(err)

        return out

    def _create_record(self, name, field, collection_id, password_options, value):
        """Create a new record with the field as a key and either an autogenerated or preset value"""

        value = self._generate_password(password_options) if value is None else value

        out, err = self._run(['get', 'template', 'item'])
        if err:
            raise BitwardenException(err)

        template = AnsibleJSONDecoder().raw_decode(out)[0]
        template['name'] = name
        if collection_id:
            template['collectionIds'] = [collection_id]
        ...
        display.v("Create new item %s" % template.get('name'))

        b64_template = b64encode(to_bytes(AnsibleJSONEncoder().encode(template)))

        out, err = self._run(['create', 'item'], stdin=b64_template)
        if err:
            raise BitwardenException(err)

        record = AnsibleJSONDecoder().raw_decode(out)[0]
        if field is None:
            return record
        return value
    ...
    def get_field(self, field, search_value, search_field="name", collection_id=None, generate=None, set_=None):
        """Return a list of the specified field for records whose search_field match search_value
        and filtered by collection if collection has been provided.

        If field is None, return the whole record for each match.
        """
        edit_record = generate is not None or set_ is not None

        matches = self._get_matches(search_value, search_field, collection_id)
        if not matches and edit_record:
            return [self._create_record(search_value, field, collection_id, generate, set_)]

        if not field:
            return matches
        field_matches = []
        for match in matches:
            display.vvvv("Looking in match %s for field %s..." % (match.get('name'), field))
            if edit_record:
                field_matches.append(self._check_and_edit_match(field, generate, match, set_))
            else:
    ...
    def _check_and_edit_match(self, field, generate, match, set_):
        if field in ['username', 'password', 'uris']:
            if 'login' not in match:
                match['login'] = {}
            if field == 'uris':
                for uri in match['login'].get('uris', []):
                    if uri.get('uri') == set_:
                        return set_
                value = self._generate_password(generate) if set_ is None else set_
                match['login']['uris'] = match['login'].get('uris', []) + [{'uri': value}]
                self._edit_record(match['id'], match)
                display.v("  added uri=%s ..." % value)
                return value
            else:
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        generate = self.get_option('generate_if_missing')
        set_ = self.get_option('set')
        _bitwarden.session = self.get_option('bw_session')

        if generate is not None and set_ is not None:
            raise AnsibleError("Options 'generate_if_missing' and 'set' are mutually exclusive.")

        if not _bitwarden.unlocked:
            raise AnsibleError("Bitwarden Vault locked. Run 'bw unlock'.")

        return [_bitwarden.get_field(field, term, search_field, collection_id, generate, set_) for term in terms]
