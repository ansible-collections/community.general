...
class DataIntersection(object):
    ...
    def __init__(self, original, modifs, present=True,
        ...
        * get :
        ...
        Starting from two similar data structure, one acting as base and second
        acting as changes to adding, updating and removing keys/elements to the
        base depending on their path, produce a new data structure which is
        the intersection of the two other.

        The `base` parameter is a data structure of type `Mapping` or
        `Sequence`. It's acting as base for creating the new data structure.
        Each of its elements will be intersected with those in `changes`.

        `changes` is also a data structure of type `Mapping` or `Sequence`. It
        contains elements that must be added, updated or removed depending
        on their path and their value when they are intersected with those of
        `base`.

        `base` and `changes` must be of same type (either both are type of
        `Mapping` or both are type of `Sequence`). Each of them can have
        childes that are lists or dictionaries which are assimilable to
        `Mapping` or `Sequence` and that will be intersected recursively.

        The type of the new produced data structure depends that the type of
        `base` and `changes` :
        - If both are type of `Mapping`, the new will be a `dict`.
        - If both are type of `Sequence`, the new will be a `list`.

        In other words, to produce the new data structure, `base` is
        recursively traversed. For each element with a key encountered in
        `base`, it looks in `changes` if an element with the same key and path
        also exists and if it's the case, it's compare their values. The way by
        this element is added or not in the new data structure depends on
        the `present`, `merge_seq_by_index`, `keep_empty` and `remove_none`
        parameters that are described bellow.

        The `present` parameter describe how to intersect `base` and `changes`.
        In all cases, all elements that only present in `base` are put as they
        are in the result. Otherwise, for a same path in both :
        - if `present=True` and an element is only present in `changes`, add it
          to the result for this path or if it is present in both but with
          different value, put in result the element of `changes` side.
        - if `present=False` and an element is present in both with the same
          value, do not put it in the result.
        In other words, if `present=True`, elements in `changes` must be found
        in the result, else they must not.
        ...
        * get :
        ...
        The `merge_seq_by_index` parameter describe how data structure that are
        assimilable to a list (that are type of `Sequence`) must be
        intersected. There are two ways to do that :
        - If `merge_seq_by_index=False`, it is simply checks if items in list on
          `changes` side are in the list on `base` side and keep them or not
          in the result depending on the value of `presence` argument. This
          be useful when you need to check the presence or the absence of a
          value in a list, but this can not check list recursively.
        - If `merge_seq_by_index=True`, the values for each list item on both
          side are compared depending on their index. In fact, before
          intersecting the two list, they are previously converted into a
          dictionary with their index as keys. This is useful when
          you want check items in the list recursively but that mean you need
          to ensure that the intersected elements share the same position in
        ...
        * and this `changes` :
          `['Z', 'B', {'C': '9', 'D': '2'}, {'E': '3'}]`
        * when `present=True`, get :
          `['Z', 'B', {'C': '9', 'D': '2'}, {'E': '3'}]`
        * or when `present=False`, get :
          `['A', {'C': '1'}]`

        The `keep_empty` parameter allow you to tell if empty lists or
        dictionaries must be kept in the result or not. It has no effect when
        `present=True`.  When `present=False`, after that a list or a
        dictionary in `base` has been intersected with another in `choices` and
        if no values are kept, this results an empty structure. If `keep_empty`
        is `True`, elements with these empty structures are kept in the result
        else they are simply removed.
        ...
        * and this `changes` :
          `{'A': {'AA': '1'}, 'B': ['2', '3']}`
        * when `keep_empty=False`, get :
          `{'C': '4'}`
        * or when `keep_empty=True`, get :
          `{'A': {}, 'B': [], 'C': '4'}`

        The `remove_none` parameter change the way by the `None` value in
        `changes` are treated.
        - When is `False` the `None` value are ignored. This may be useful with
          `merge_seq_by_index=True` when you only want to taking care of values
          that are at a specific positions in the list. You can set the value
          of elements that are before to `None` in `changes` side and, they will
          be ignored.
        - When is `True`, the elements that have `None` value in `changes` are
          not kept in the result. This is useful when you want to ensure that a
          key is absent to the result without having worries about its value in
          `base`.

        Example with values ignored at specific position in list with
        `remove_none=False`, `merge_seq_by_index=True` :
        * with this `base` :
          `['A', 'B', 'C', {'DA': '1', 'DB': '2'}, 'E']`
        * and this `changes` :
          `[None, None, 'Z', {'DA': '1', 'DB': None}, 'E']`
        * when `present=True', get :
          `['A', 'B', 'Z', {'DA': '1', 'DB': '2'}, 'E']`
        * when `present=False', get :
          `['A', 'B', 'C', {'DB': '2'}]`

        Example of key removing with `remove_none=True` and `present=False` :
        * with this `base` :
         `{'A': '1', 'B': '2', 'C': {'CA': '3', 'CB': '4'}, 'D': ['DA', 'DB']}`
        * and this `changes` :
          `{'B': None, 'C': {'CB': None}, 'D': None}`
