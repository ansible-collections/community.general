...
def github_common_argument_spec():
    return dict(
        ...
        server=dict(type='str', default='https://api.github.com', aliases=['github_url']),
    )

@six.add_metaclass(abc.ABCMeta)
class GitHubBase(object):

    def __init__(self, username=None, password=None, token=None, otp=None, server='https://api.github.com', repo=None, organization=None):
    ...
    def auth(self):
        try:
            ...
            raise GithubError("Could not connect to GitHub at {0}: {1}".format(self.server, to_native(err)))
    ...
    def repository(self):
        try:
            ...
            raise GitHubBadCredentialsError("Unable to authenticate to GiHub at {0}: {1}".format(self.server, to_native(err)))
