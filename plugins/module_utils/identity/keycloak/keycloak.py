...
def _token_request(module_params, payload):
    ...
    try:
        ...
        raise KeycloakError(
            f'API returned invalid JSON when trying to obtain access token from {auth_url}: {e}')
    except KeyError:
        raise KeycloakError(
            f'API did not include access_token field in response from {auth_url}')
...
class KeycloakAPI:
    ...
    def get_realm_info_by_id(self, realm='master'):
        ...
        try:
            ...
            if e.code == 404:
                ...
                self.fail_request(e, msg=f'Could not obtain realm {realm}: {e}',
                                  exception=traceback.format_exc())
        except ValueError as e:
            self.module.fail_json(msg=f'API returned incorrect JSON when trying to obtain realm {realm}: {e}',
                                  exception=traceback.format_exc())
        except Exception as e:
            self.module.fail_json(msg=f'Could not obtain realm {realm}: {e}',
                                  exception=traceback.format_exc())
    ...
    def update_realm(self, realmrep, realm="master"):
        ...
        try:
            ...
            self.fail_request(e, msg=f'Could not update realm {realm}: {e}',
                              exception=traceback.format_exc())
    ...
    def create_realm(self, realmrep):
        ...
        try:
            ...
            self.fail_request(e, msg=f"Could not create realm {realmrep['id']}: {e}",
                              exception=traceback.format_exc())
    ...
    def delete_realm(self, realm="master"):
        ...
        try:
            ...
            self.fail_request(e, msg=f'Could not delete realm {realm}: {e}',
                              exception=traceback.format_exc())
    ...
    def create_empty_auth_flow(self, config, realm='master'):
        ...
        try:
            ...
            self.fail_request(e, msg=f"Could not create empty authentication flow {config['alias']} in realm {realm}: {e}")
    ...
    def create_authz_custom_policy(self, policy_type, payload, client_id, realm):
        ...
        try:
            ...
            self.fail_request(e, msg=f"Could not create permission {payload['name']} for client {client_id} in realm {realm}: {e}")
    ...
    def remove_authz_custom_policy(self, policy_id, client_id, realm):
        ...
        try:
            ...
            self.fail_request(e, msg=f'Could not delete custom policy {id} for client {client_id} in realm {realm}: {e}')
    ...
    def update_authz_permission(self, payload, permission_type, id, client_id, realm):
        ...
        try:
            ...
            self.fail_request(e, msg=f"Could not create update permission {payload['name']} for client {client_id} in realm {realm}: {e}")
