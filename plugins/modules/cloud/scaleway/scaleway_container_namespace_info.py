...
description:
  - This module return info on container namespace on Scaleway account.
...
RETURN = '''
container_namespace:
    description: The container namespace informations.
    returned: always
    type: dict
    sample: {
      "description": "",
...
def info_strategy(api, wished_cn):
    ...
    if not response.ok:
        api.module.fail_json(msg='Error getting container namespaces [{0}: {1}]'.format(
