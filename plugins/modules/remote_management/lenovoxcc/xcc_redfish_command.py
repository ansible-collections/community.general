        ...
        for response in responses:
            ...
            if not uri:
                return {'ret': False,
                        ...
                        "%s" % ('supporting ' + str(media_types) if media_types else '')}
            ...
            if 'target' not in action:
                return {'ret': False,
                        'msg': "target URI missing from Action #VirtualMedia.InsertMedia"}
    ...
    def virtual_media_eject_one(self, image_url):
        ...
        for response in responses:
            ...
            if uri and eject:
                if ('Actions' not in data or
                    ...
                    if 'allow' in h:
                        ...
                        if 'PATCH' not in methods:
                            # if Allow header present and PATCH missing, return error
                            return {'ret': False,
                                    'msg': "%s action not found and PATCH not allowed" % '#VirtualMedia.EjectMedia'}
                    ...
                    if 'target' not in action:
                        return {'ret': False,
                                'msg': "target URI property missing from Action #VirtualMedia.EjectMedia"}
                    action_uri = action['target']
                ...
                return {'ret': True, 'changed': False,
                        'msg': "VirtualMedia image '%s' already ejected" % image_url}
            else:
