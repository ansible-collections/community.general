    ...
    def delete_group(self, force=False):
        ...
        if not force and len(group.projects.list(all=False)) >= 1:
            self._module.fail_json(
                msg="There are still projects in this group. These needs to be moved or deleted before this group can be removed. Use 'force_delete' to 'true' to force deletion of existing projects.")
        else:
            if self._module.check_mode:
                return True
