...
class PacemakerCluster(StateModuleHelper):
    ...
    def state_online(self):
        ...
        if get_pacemaker_maintenance_mode(self._maintenance_mode_runner):
            with self._maintenance_mode_runner('state name', output_process=self._process_command_output(True), check_mode_skip=True) as ctx:
              ctx.run(name="maintenance-mode=false")
    ...
    def state_cleanup(self):
        with self.runner('state', output_process=self._process_command_output(True, ""), check_mode_skip=True) as ctx:
