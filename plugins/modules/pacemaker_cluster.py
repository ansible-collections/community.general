...
class PacemakerCluster(StateModuleHelper):
    module = dict(
        argument_spec=dict(
            state=dict(type="str", choices=["cleanup", "offline", "online", "restart", "maintenance", "unmaintenance"], required=True),
    ...
    def state_unmaintenance(self):
        with self.runner(
            ...
            ctx.run(cli_action="property", name="maintenance-mode=false")
