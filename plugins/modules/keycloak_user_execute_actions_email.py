...
description:
  ...
  - You must supply either the user's C(id) or C(username). Supplying only C(username) causes an extra lookup call.
...
options:
  ...
  redirect_uri:
    description:
      - Optional redirect URI. Must be valid for the given client if C(client_id) is set.
    aliases: [redirectUri]
    type: str
  lifespan:
    description:
