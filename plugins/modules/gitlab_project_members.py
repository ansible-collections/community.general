...
def main():
    ...
    if len(gitlab_users_access) == 1 and not purge_users:
        ...
        members = [
            project.get_member_in_a_project(gitlab_project_id, project.get_user_id(gitlab_users_access[0]["name"]))
        ]
        ...
        if not is_user_a_member:
            ...
            # state as absent
                ...
                changed_users.append(
                    f"User, '{gitlab_user['name']}', is not a member in the project. No change to report"
                )
                changed_data.append(
                    {
                        "gitlab_user": gitlab_user["name"],
        ...
        # in case that a user is a member
            ...
            if state == "present":
                ...
                if user_access_level == gitlab_user["access_level"]:
                    changed_users.append(
                        f"User, '{gitlab_user['name']}', is already a member in the project. No change to report"
                    )
                    changed_data.append(
                        {
                            "gitlab_user": gitlab_user["name"],
        ...
        for member in members:
            if (
                ...
                try:
                    ...
                    changed = True
                    changed_users.append(
                        f"Successfully removed user '{member.username}', from project. Was not in given list"
                    )
                    changed_data.append(
                        {
