...
def delete_user_data(compute_api, server_id, key):
    ...
    if not response.ok:
        msg = "Error during user_data deleting: ({}) {}".format(*response.status_code), response.body
        compute_api.module.fail_json(msg=msg)
...
def core(module):
    ...
    if not user_data_list.ok:
        msg = "Error during user_data fetching: {} {}".format(*user_data_list.status_code), user_data_list.body
        compute_api.module.fail_json(msg=msg)
