...
notes:
  - To obtain an C(api_token), login to Tanzu Mission Control, click on your user, then I(Settings) and then I(Generate an
    API token).
...
class TMCClusterGroupSecret(TMCSecret):
    ...
    def _generate_query_params(self):
        return {"fullName.namespaceName": self._cluster_namespace}
    ...
    def _generate_export_url(self, api_host):
        return "https://{}/v1alpha1/clustergroups/{}/namespace/secretexports".format(
            api_host, self._cluster_group
        )
...
def exchange_token(api_token):
    ...
    exchange_result = post(
        exchange_url, data, headers={"Content-Type": content_type}, timeout=5000
    )
    ...
    payload = {
        "auths": {
            registry_host: {
                ...
                "auth": b64encode(
                    "{}:{}".format(registry_username, registry_password).encode()
                ).decode(),
            }
        }
    }
...
def main():
    ...
    if access_token is None:
        module.fail_json(
            msg="Failed to exchange token with TMC APIs -- check that the token you provided is valid."
        )
    ...
    if cluster_name is not None:
        ...
        tmc_secret = TMCClusterGroupSecret(
            str(cluster_group), cluster_namespace, secret_name, secret_type
        )
