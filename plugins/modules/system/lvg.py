        ...
        if state == 'present':
            ...
            if module.check_mode:
                ...
                for current_dev in dev_list:
                    rc, unused_var, err = module.run_command([pvcreate_cmd] + pvoptions + ['-f', str(current_dev)])
                    if rc == 0:
                        changed = True
                    else:
                        module.fail_json(msg="Creating physical volume '%s' failed" % current_dev, rc=rc, err=err)
                vgcreate_cmd = module.get_bin_path('vgcreate')
                rc, unused_var, err = module.run_command([vgcreate_cmd] + vgoptions + ['-s', pesize, vg] + dev_list)
                if rc == 0:
                    changed = True
                else:
        ...
        if state == 'absent':
            if module.check_mode:
                ...
                if this_vg['lv_count'] == 0 or force:
                    ...
                    rc, unused_var, err = module.run_command("%s --force %s" % (vgremove_cmd, vg))
                    if rc == 0:
                        module.exit_json(changed=True)
                    else:
                ...
                for device in current_devs:
                    ...
                    if (dev_size - (pe_start + pv_size)) > vg_extent_size:
                        if module.check_mode:
                            ...
                            rc, unused_var, err = module.run_command([pvresize_cmd, device])
                            if rc != 0:
        ...
        if devs_to_add or devs_to_remove:
            if module.check_mode:
                ...
                if devs_to_add:
                    ...
                    rc, unused_var, err = module.run_command("%s %s %s" % (vgextend_cmd, vg, devs_to_add_string))
                    if rc == 0:
                        changed = True
                    else:
                ...
                if devs_to_remove:
                    ...
                    rc, unused_var, err = module.run_command("%s --force %s %s" % (vgreduce_cmd, vg, devs_to_remove_string))
                    if rc == 0:
                        changed = True
                    else:
