...
options:
  state:
    ...
    type: str
    choices:
      created: Takes a new snapshot of O(source).
      deleted: Deletes the snapshot identified by O(snapshot_id).
      expired: >-
        Applies retention policy and deletes snapshots that no longer meet it.
        This is a dry-run unless O(delete=true).
      listed: Lists snapshots for O(source) or all sources when O(source) is not set.
      verified: >-
        Verifies the integrity of snapshots identified by O(snapshot_id),
