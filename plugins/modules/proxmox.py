...
def main():
    ...
    if state == 'present' and clone is None:
        try:
            if proxmox.get_vm(vmid, ignore_missing=True) and not module.params['force']:
                module.exit_json(changed=False, vmid=vmid, taskid=taskid, msg="VM with vmid = %s is already exists" % vmid)
            # If no vmid was passed, there cannot be another VM named 'hostname'
            if (not module.params['vmid'] and
                    proxmox.get_vmid(hostname, ignore_missing=True) and
                    not module.params['force']):
                vmid = proxmox.get_vmid(hostname)
                module.exit_json(changed=False, vmid=vmid, taskid=taskid, msg="VM with hostname %s already exists and has ID number %s" % (hostname, vmid))
            elif not proxmox.get_node(node):
                module.fail_json(vmid=vmid, taskid=taskid, msg="node '%s' not exists in cluster" % node)
        ...
        try:
            ...
            module.exit_json(changed=True, vmid=vmid, taskid=taskid, msg="Deployed VM %s from template %s" % (vmid, module.params['ostemplate']))
        except Exception as e:
            module.fail_json(vmid=vmid, taskid=taskid, msg="Creation of %s VM %s failed with exception: %s" % (VZ_TYPE, vmid, e))
    ...
    # Clone a container
        ...
        try:
            ...
            module.exit_json(changed=True, vmid=vmid, taskid=taskid, msg="Cloned VM %s from %s" % (vmid, clone))
        except Exception as e:
            module.fail_json(vmid=vmid, taskid=taskid, msg="Cloning %s VM %s failed with exception: %s" % (VZ_TYPE, vmid, e))

    elif state == 'started':
        try:
            vm = proxmox.get_vm(vmid)
            if getattr(proxmox.proxmox_api.nodes(vm['node']), VZ_TYPE)(vmid).status.current.get()['status'] == 'running':
                module.exit_json(changed=False, vmid=vmid, taskid=taskid, msg="VM %s is already running" % vmid)

            if proxmox.start_instance(vm, vmid, timeout):
                module.exit_json(changed=True, vmid=vmid, taskid=taskid, msg="VM %s started" % vmid)
        except Exception as e:
            module.fail_json(vmid=vmid, taskid=taskid, msg="starting of VM %s failed with exception: %s" % (vmid, e))
        ...
        try:
            ...
            if getattr(proxmox.proxmox_api.nodes(vm['node']), VZ_TYPE)(vmid).status.current.get()['status'] == 'mounted':
                if module.params['force']:
                    if proxmox.umount_instance(vm, vmid, timeout):
                        module.exit_json(changed=True, vmid=vmid, taskid=taskid, msg="VM %s is shutting down" % vmid)
                else:
                    module.exit_json(changed=False, vmid=vmid, taskid=taskid, msg=("VM %s is already shutdown, but mounted. "
            ...
            if getattr(proxmox.proxmox_api.nodes(vm['node']), VZ_TYPE)(vmid).status.current.get()['status'] == 'stopped':
                module.exit_json(changed=False, vmid=vmid, taskid=taskid, msg="VM %s is already shutdown" % vmid)

            if proxmox.stop_instance(vm, vmid, timeout, force=module.params['force']):
                module.exit_json(changed=True, vmid=vmid, taskid=taskid, msg="VM %s is shutting down" % vmid)
        except Exception as e:
            module.fail_json(vmid=vmid, taskid=taskid, msg="stopping of VM %s failed with exception: %s" % (vmid, e))
        ...
        try:
            ...
            if vm_status in ['stopped', 'mounted']:
                module.exit_json(changed=False, vmid=vmid, taskid=taskid, msg="VM %s is not running" % vmid)
