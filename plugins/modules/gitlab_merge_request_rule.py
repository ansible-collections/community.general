...
options:
  ...
  groups:
    description:
      - List of group names (full path) or group ids of members who can approve merge requests for this rule.
    type: list
  protected_branches:
    description:
      - List of protected branch names or branch ids this rule applies to.
...
class GitlabMergeRequestRule(object):
    def __init__(self, module, gitlab_instance, group, project, rule_name, rule_approvals_required=0,
                 rule_groups=None, rule_user_ids=None, rule_usernames=None, rule_branches=None,
        ...
        self.rule_groups: int | list = self.build_groups(self._string_to_set(rule_groups))
        self.rule_usernames: str = self._string_to_set(rule_usernames) if rule_usernames is not None else []
        self.rule_user_ids: str | int | list = self._string_to_set(rule_user_ids) if rule_user_ids is not None else []
        self.rule_users: str | int | list = self.build_users()
        self.rule_apporvals_required: str = rule_approvals_required
        self.rule_branches: str | int | list = self._gather_ids_for_protected_branches(rule_branches) if rule_branches is not None else []
        self.rule_all_protected_branches: bool = rule_all_protected_branches
        self.rule_exists, self.existing_rule = self.check_rule_exists(rule_name)
        if self.rule_exists:
            self._updates = False
            self.rule_updated: bool = self.check_for_updates_to_existing_rule()
    ...
    def _gather_ids_for_protected_branches(self, wanted_branches):
        ...
        for b in string_branches:
            ...
            if not b.isdigit():
                ...
                except gitlab.GitlabGetError as gerr:
                    print(f'[{b}] {gerr.error_message}')
            final_branches.add(_id)
        return final_branches

    def _check_protected_branch_exists(self, protected_branch):
        try:
            return next(pb.id for pb in self.protected_branches if pb.name == protected_branch)
    ...
    def build_groups(self, wanted_groups):
        ...
        for grp in wanted_groups:
            try:
                ...
                self._module.fail_json(msg=f'Failed to determine specified group ({grp})', err=err)
        return final_groups
    ...
    def build_users(self):
        ...
        for usr in combined_users:
            try:
                ...
                self._module.fail_json(msg=f'Failed to determine specified user ({usr})', err=err)
        return final_users
    ...
    def _generated_data(self):
        ...
        _protected_branch_ids = list(self.rule_branches) if self.rule_branches is not None else []
    ...
    def create_approval_rule(self):
        if self._module.check_mode:
            return True
        return self.project.approvalrules.create(
    ...
    def update_approval_rule(self):
        ...
        self.existing_rule.protected_branch_ids = list(self.rule_branches) if self.rule_branches is not None else []
        self.existing_rule.user_ids = list(self.rule_users) if self.rule_users is not None else []
        self.existing_rule.group_ids = list(self.rule_groups) if self.rule_groups is not None else []
        self.existing_rule.applies_to_all_protected_branches = bool(self.rule_all_protected_branches)
        return self.existing_rule.save()

    def _check_changed_members(self, updated_bool, new_members, previous_members) -> bool:
    ...
    def check_for_updates_to_existing_rule(self) -> bool:
        updated = False
        if self.existing_rule.applies_to_all_protected_branches != self.rule_all_protected_branches:
            updated = True
        if int(self.existing_rule.approvals_required) != int(self.rule_apporvals_required):
            updated = True

        # Check for changed / removed protected branches
        updated = self._check_changed_members(updated, self.rule_branches, self.existing_rule.protected_branches)
        updated = self._check_changed_members(updated, self.existing_rule.protected_branches, self.rule_branches)

        # Check for changed /removed users
        updated = self._check_changed_members(updated, self.rule_users, self.existing_rule.users)
        updated = self._check_changed_members(updated, self.existing_rule.users, self.rule_users)

        # Check for changed / removed groups
        updated = self._check_changed_members(updated, self.rule_groups, self.existing_rule.groups)
        updated = self._check_changed_members(updated, self.existing_rule.groups, self.rule_groups)
        ...
        if isinstance(self.given_user, str) and not self.given_user.isdigit():
            try:
                ...
                if num_found == 1:
                    ...
                    self._module.fail_json(msg=f'More than one matching user found, unable to determine correct user ({self.given_user})', query_result=found_user)
                else:
                    self._module.fail_json(msg=f'User could not be found ({self.given_user})')
            except gitlab.GitlabError as gerr:
                    self._module.fail_json(msg=f'User could not be found ({self.given_user})', err=gerr.error_message)
    ...
    def get(self):
        if isinstance(self.given_group, int) or self.given_group.isdigit():
            try:
                ...
                self._module.fail_json(msg=f'Group could not be found ({self.given_group})', err=gerr.error_message)
            ...
            try:
                ...
                if num_groups == 1:
                    ...
                    self._module.fail_json(msg=f'More than one matching group found, unable to determine correct group ({self.given_group})', query_result=found_groups)
                else:
                    self._module.fail_json(msg=f'Group could not be found ({self.given_group})')
            except gitlab.GitlabError as gerr:
                self._module.fail_json(msg=f'Group could not be found ({self.given_group})', err=gerr.error_message)
...
def run_module():
    ...
    module_args.update(
        ...
        usernames=dict(type='list', default=[], required=False),
        user_ids=dict(type='list', default=[], required=False),
        groups=dict(type='list', default=[], required=False),
        protected_branch=dict(type='str', default=None, required=False),
        applies_to_all_protected_branches=dict(type='bool', default=False, required=False),
    ...
    if not gmmr.rule_exists:
        if module.params['state'] == 'absent':
            module.exit_json(
                changed=False, msg=f'Merge Request Rule ({module.params["rule_name"]}) does not exist in {module.params["project_name"]}, nothing to delete.'
            )

        elif module.params['state'] == 'present':
            gmmr.create_approval_rule()
            gmmr.rules = gmmr.list_approval_rules()
            _, new_mmr = gmmr.check_rule_exists(gmmr.rule_name)
            module.exit_json(
                changed=True, msg=f'Merge Request Rule ({module.params["rule_name"]}) successfully created.',
                mrr=new_mmr.asdict()
            )
        ...
        if module.params['state'] == 'absent':
            ...
            module.exit_json(
                changed=True, msg=f'Merge Request Rule ({module.params["rule_name"]}) was deleted from {module.params["project_name"]}.'
            )
            ...
            _, new_mmr = gmmr.check_rule_exists(gmmr.rule_name)
            module.exit_json(
                changed=True, msg=f'Merge Request Rule ({module.params["rule_name"]}) changes were found, updated rule.',
                mrr=new_mmr.asdict()
            )

        else:
            module.exit_json(
                changed=False, msg='No changes were detected.',
                mrr=gmmr.existing_rule.asdict()
            )
