...
def main():
    ...
    try:
        ...
        if not domain:
            domain_list = []
            domains_pagination = client.domains.list_domains(account.id).pagination
            for page in range(1, domains_pagination.total_pages + 1):
                page_data = client.domains.list_domains(account.id, page=page).data
        ...
        if domain and record is not None:
            ...
            records_list = []
            records_pagination = client.zones.list_records(account.id, dr.name, filter={'name': record}).pagination
            for page in range(1, records_pagination.total_pages + 1):
                page_data = client.zones.list_records(account.id, dr.name, filter={'name': record}, page=page).data
                records_list.extend(page_data)

            records = [r for r in records_list]
        ...
        if domain and record_ids:
            dr = client.domains.get_domain(account.id, domain).data
            current_records = [str(r.id) for r in client.zones.list_records(account.id, dr.name).data]
