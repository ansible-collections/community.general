...
class CallbackModule(CallbackBase):
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        self.logger.error(
            mask_secrets("%s ansible-command: task execution FAILED; host: %s; message: %s"),
            self.hostname,
            host,
            self._dump_results(mask_secret_values(res)),
        )
    ...
    def v2_runner_on_ok(self, result):
        ...
        if result._task.action != "gather_facts" or self.get_option("setup"):
            self.logger.info(
                mask_secrets("%s ansible-command: task execution OK; host: %s; message: %s"),
                self.hostname,
                host,
                self._dump_results(mask_secret_values(res)),
            )
    ...
    def v2_runner_on_skipped(self, result):
        host = result._host.get_name()
        self.logger.info(
            mask_secrets("%s ansible-command: task execution SKIPPED; host: %s; message: %s"),
            self.hostname,
            host,
            "skipped",
        )
    ...
    def v2_runner_on_unreachable(self, result):
        ...
        host = result._host.get_name()
        self.logger.error(
            mask_secrets("%s ansible-command: task execution UNREACHABLE; host: %s; message: %s"),
            self.hostname,
            host,
            self._dump_results(mask_secret_values(res)),
        )
    ...
    def v2_playbook_on_import_for_host(self, result, imported_file):
        host = result._host.get_name()
        self.logger.info(
            mask_secrets("%s ansible-command: playbook IMPORTED; host: %s; message: imported file %s"),
            self.hostname,
            host,
    ...
    def v2_playbook_on_not_import_for_host(self, result, missing_file):
        host = result._host.get_name()
        self.logger.info(
            mask_secrets("%s ansible-command: playbook NOT IMPORTED; host: %s; message: missing file %s"),
            self.hostname,
            host,
